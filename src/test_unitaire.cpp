...
UnitTest::UnitTest(LGFX &tft) : tft(tft)
{
}

void UnitTest::testSD()
{
  // Initialise le bus SPI partagé avec l'écran
  SPI.begin(PIN_SCLK, PIN_MISO, PIN_MOSI);

  Serial.println(F("[TEST] Initialisation de la SD"));

  Serial.println(!SD.begin(PIN_CS) ? F("[TEST] Carte SD: ❌") : F("[TEST] Carte SD: ✅"));
}

void UnitTest::testWifi(const char *ssid, const char *password)
{
  Serial.println(F("[TEST] Connexion Wi-Fi"));
  WiFi.mode(WIFI_STA);
  ...
  WiFi.status() == WL_CONNECTED
    ? (String(F("[TEST] Wi-Fi: ✅\n[INFO] Adresse IP: ")) + WiFi.localIP().toString())
    : String(F("[TEST] Wi-Fi: ❌"))
);

}

void UnitTest::testWebSocket(const char *host, uint16_t port)
{
  Serial.print(F("[TEST] Connexion WebSocket: "));


  if (WiFi.status() != WL_CONNECTED)
  {
    Serial.println(F("[ERREUR] Wi-Fi: ❌"));
    return;
  }
  ...
  unsigned long start = millis();
  while (!ws.isConnected() && millis() - start < 5000){
    ws.loop();
  }

  Serial.println(ws.isConnected()
    ? Serial.printf("[TEST] WebSocket: ✅\n[INFO] Adresse IP: %s:%d\n", host, port)
    : Serial.println(F("[TEST] WebSocket: ❌")));

  ws.disconnect();
}

void UnitTest::testAffichage()
{
